...
{ 
	const I off = p.srch.offs,val = p.srch.val;
	const S cur = p.rsdt[off];
	...
	if(!p.srch.dir) {
		if(ol >= 0) {
			p.srch.dif = off-ol;
			if(or >= 0 && abs(off-ol) < abs(or-off)) p.srch.dif = or-off;
		}
		else 
...
{ 
	...
	if(src.Vectors() > 1) 
	...
	else if(arg.IsList() && arg.GetList().Count() >= 1) {
		I fr = src.ChkFrames();
		...
		if(vecs) {
			p.srch.val = flx::GetAFloat(arg.GetList()[0]);
